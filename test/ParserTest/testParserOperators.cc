...
TEST_F( ParserOperatorsTest, assignment ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_ASSIGN, { TokenType::IDENTIFIER, "y" } };
...
TEST_F( ParserOperatorsTest, addition_assignment ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_ADD_ASSIGN, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x += y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, subtraction_assignment ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_SUB_ASSIGN, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x -= y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, multiplication_assignment ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_MUL_ASSIGN, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x *= y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, division_assignment ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_DIV_ASSIGN, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x /= y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, modulo_assignment ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_MOD_ASSIGN, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x %= y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, logical_or ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_OR, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
...
TEST_F( ParserOperatorsTest, logical_and ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_AND, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
...
TEST_F( ParserOperatorsTest, equality ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_EQ, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x == y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, inequality ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_NEQ, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x != y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, relational_less_than ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_LT, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x < y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, relational_less_or_equal ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_LEQ, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x <= y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, relational_greater_than ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_GT, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x > y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, relational_greater_or_equal ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_GEQ, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x >= y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, additive_addition ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_PLUS, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x + y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, additive_subtraction ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_MINUS, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x - y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, additive_concatenation ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_CONCAT, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x ++ y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, additive_diff ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_DIFF, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x -- y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, multiplicative_multiplication ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_MUL, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x * y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, multiplicative_division ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_DIV, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x / y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, multiplicative_modulo ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_MOD, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x % y}}", std::move( res ) );
}

TEST_F( ParserOperatorsTest, unary_negation_arithmetical ) {
  std::vector<TokenInitializer> init = { TokenType::OP_MINUS, { TokenType::IDENTIFIER, "x" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{-x}}", std::move( res ) );
}

TEST_F( ParserOperatorsTest, unary_negation_logical ) {
  std::vector<TokenInitializer> init = { TokenType::OP_NOT, { TokenType::IDENTIFIER, "x" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{not x}}", std::move( res ) );
}

TEST_F( ParserOperatorsTest, unary_reverse ) {
  std::vector<TokenInitializer> init = { TokenType::OP_REV, { TokenType::IDENTIFIER, "x" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{@x}}", std::move( res ) );
}

TEST_F( ParserOperatorsTest, unary_len ) {
  std::vector<TokenInitializer> init = { TokenType::OP_LEN, { TokenType::IDENTIFIER, "x" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{$x}}", std::move( res ) );
}

TEST_F( ParserOperatorsTest, cast_to ) {
  std::vector<TokenInitializer> init = { { TokenType::IDENTIFIER, "x" }, TokenType::KW_CAST_TO, TokenType::T_INT };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
...
TEST_F( ParserOperatorsTest, access_index ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::LBRACKET, { TokenType::IDENTIFIER, "y" }, TokenType::RBRACKET };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x[y]}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, access_map ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_MAP, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x -> y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, access_filter ) {
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, TokenType::OP_FILTER, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x ? y}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, multiplicative_before_additive_overriden_by_parentheses ) {
  ...
  std::vector<TokenInitializer> init = {
      ...
      TokenType::RPAREN, { TokenType::OP_MUL },          { TokenType::IDENTIFIER, "z" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{{x + y} * z}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, unary_before_multiplicative ) {
  // x * -y
  std::vector<TokenInitializer> init = {
      { TokenType::IDENTIFIER, "x" }, { TokenType::OP_MUL }, { TokenType::OP_MINUS }, { TokenType::IDENTIFIER, "y" } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{x * {-y}}}", std::move( res ) );
}
...
TEST_F( ParserOperatorsTest, cast_before_unary_overriden_by_parentheses ) {
  // (-x) cast_to float
  std::vector<TokenInitializer> init = {
      TokenType::LPAREN,         { TokenType::OP_MINUS }, { TokenType::IDENTIFIER, "x" }, TokenType::RPAREN,
...
TEST_F( ParserOperatorsTest, access_before_cast ) {
  // x[y] cast_to int
  std::vector<TokenInitializer> init = { { TokenType::IDENTIFIER, "x" }, TokenType::LBRACKET,
                                         { TokenType::IDENTIFIER, "y" }, TokenType::RBRACKET,
                                         TokenType::KW_CAST_TO,          TokenType::T_INT };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{{x[y]} cast_to INT}}", std::move( res ) );
}

TEST_F( ParserOperatorsTest, operaor_map_chain ) {
  std::vector<TokenInitializer> init = { { TokenType::IDENTIFIER, "arr" },  { TokenType::OP_MAP },
...
TEST_F( ParserOperatorsTest, operaor_filter_chain ) {
  std::vector<TokenInitializer> init = { { TokenType::IDENTIFIER, "arr" },  { TokenType::OP_FILTER },
                                         { TokenType::IDENTIFIER, "foo1" }, { TokenType::OP_FILTER },
                                         { TokenType::IDENTIFIER, "foo2" }, { TokenType::NEWLINE },
                                         { TokenType::END_OF_FILE } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{{arr ? foo1} ? foo2}}", std::move( res ) );
...
TEST_F( ParserOperatorsTest, additive_left_associativity ) {
  std::vector<TokenInitializer> init = {
      { TokenType::INT_LITERAL, 10 }, { TokenType::OP_PLUS }, { TokenType::INT_LITERAL, 5 }, { TokenType::OP_PLUS },
...
TEST_F( ParserOperatorsTest, multiplicative_left_associativity ) {
  std::vector<TokenInitializer> init = {
      { TokenType::INT_LITERAL, 10 }, { TokenType::OP_MUL },  { TokenType::INT_LITERAL, 5 }, { TokenType::OP_MUL },
      { TokenType::INT_LITERAL, 2 },  { TokenType::NEWLINE }, { TokenType::END_OF_FILE } };
  std::string res = initTokensBuildProgramAndSerialize( std::move( init ) );
  EXPECT_EQ( "{{{10 * 5} * 2}}", std::move( res ) );
}
